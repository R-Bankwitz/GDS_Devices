...
class waveguide_integrated_Filter:
    ...
    def create(self, x, y, coupler_params, device, device_width, device_length, number_of_filters):
        ...
        print(wg1.current_port.get_parameters()["origin"])

        txt = ('number_of_filters: %s ') \
              % (number_of_filters)  #
        ...
        for i in range(number_of_filters):
            filter_cell = Cell("filter_%s_%s_%s_%s%s" %(number_of_filters, True,i,x,y))
